      ...
      await database.withTransactionAsync(async () => {
        ...
        for (const session of snapshot.sessions) {
          ...
          await database.runAsync(`INSERT OR IGNORE INTO focus_sessions
            (id, task_id, mode, timer_mode, started_at, ended_at, outcome, failure_reason,
            ...
            session.outcome === 'completed' ? 'completed' : 'exited', session.failureReasonText,
...
function entityType(operation: SyncOperation) { return operation.type.startsWith('session.') ? 'session' : 'task'; }
