...
export function TaskList({ tasks, onStart, onGoalProgress }: { tasks: Task[]; onStart: (taskId: string, mode: SessionMode) => void; onGoalProgress: (taskId: string, amount: number) => Promise<void> }) {
  return <View className="gap-3">{tasks.length === 0 ? <Card><Card.Body><Card.Title>还没有任务</Card.Title><Card.Description>创建一个最小任务，开始今天的闭环。</Card.Description></Card.Body></Card> : tasks.map((task) => <TaskCard key={task.id} task={task} onStart={onStart} onGoalProgress={onGoalProgress} />)}</View>;
}
