...
function HabitCard({ habit, onProgress, onArchive, onOpen }: { habit: Habit; onProgress(id: string, minutes: number): Promise<void>; onArchive(id: string): Promise<ArchiveHabitResult>; onOpen?(habitId: string): void }) {
  ...
  return <Card><Card.Body className="gap-3"><View className="flex-row items-center justify-between gap-3"><View className="flex-1"><Card.Title>{habit.name}</Card.Title><Card.Description>今日 {habit.todayMinutes}/{habit.targetMinutes} 分钟 · {percent}%</Card.Description></View>{habit.forceEnabled ? <Chip color="danger" variant="soft">{habit.triggerTime} 强制触发</Chip> : null}</View><View className="flex-row flex-wrap gap-2"><Button size="sm" onPress={() => void onProgress(habit.id, 5)}>+5 分钟</Button><Button size="sm" variant="secondary" onPress={() => void onProgress(habit.id, 10)}>+10 分钟</Button>{onOpen ? <Button size="sm" variant="secondary" onPress={() => onOpen(habit.id)}>详情</Button> : null}</View><View className="flex-row items-end gap-2"><View className="flex-1"><Field label="补记分钟" value={minutes} onChange={setMinutes} keyboard="numeric" /></View><Button size="sm" variant="secondary" onPress={() => void onProgress(habit.id, Number(minutes))}>记录</Button></View><Text type="body-xs" color="muted" onPress={() => void onArchive(habit.id)}>归档习惯</Text></Card.Body></Card>;
}
