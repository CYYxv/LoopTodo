...
type TaskAiStore = { configured: boolean; client: TaskAiClient | null; materials: Record<string, AiMaterial[]>; answer: AiAnswer | null; loading: boolean; error: string | null;
...
export function createTaskAiStore(initialClient: TaskAiClient | null = null) {
  return createStore<TaskAiStore>((set, get) => ({ configured: Boolean(initialClient), client: initialClient, materials: {}, answer: null, loading: false, error: null,
    configure(baseUrl, accessToken) { set({ client: createHttpTaskAiClient(baseUrl, accessToken), configured: true, error: null }); },
    async load(taskId) { const client = get().client; if (!client) return; try { const materials = await client.listMaterials(taskId); set((state) => ({ materials: { ...state.materials, [taskId]: materials }, error: null })); } catch (error) { set({ error: message(error) }); } },
