jest.mock('heroui-native/button', () => require('./test/heroui.mock').buttonModule);
jest.mock('heroui-native/card', () => require('./test/heroui.mock').cardModule);
jest.mock('heroui-native/chip', () => require('./test/heroui.mock').chipModule);
jest.mock('heroui-native/description', () => require('./test/heroui.mock').descriptionModule);
jest.mock('heroui-native/input', () => require('./test/heroui.mock').inputModule);
jest.mock('heroui-native/label', () => require('./test/heroui.mock').labelModule);
jest.mock('heroui-native/provider', () => require('./test/heroui.mock').providerModule);
jest.mock('heroui-native/switch', () => require('./test/heroui.mock').switchModule);
jest.mock('heroui-native/text', () => require('./test/heroui.mock').textModule);
jest.mock('heroui-native/text-field', () => require('./test/heroui.mock').textFieldModule);
jest.mock('heroui-native/bottom-sheet', () => require('./test/heroui.mock').bottomSheetModule);
jest.mock('heroui-native/surface', () => require('./test/heroui.mock').surfaceModule);
jest.mock('heroui-native/pressable-feedback', () => require('./test/heroui.mock').pressableFeedbackModule);
jest.mock('heroui-native/accordion', () => require('./test/heroui.mock').accordionModule);
jest.mock('heroui-native/avatar', () => require('./test/heroui.mock').avatarModule);
jest.mock('heroui-native/list-group', () => require('./test/heroui.mock').listGroupModule);
jest.mock('heroui-native/separator', () => require('./test/heroui.mock').separatorModule);
jest.mock('@gorhom/bottom-sheet', () => require('./test/heroui.mock').gorhomBottomSheetModule);
jest.mock('react-native-safe-area-context', () => require('./test/safe-area.mock'));
jest.mock('expo-video', () => {
  const { View } = require('react-native');
  return { useVideoPlayer: jest.fn(() => ({})), VideoView: View };
});
jest.mock('react-native-webview', () => {
  const { View } = require('react-native');
  return { WebView: View };
});
jest.mock('expo-secure-store', () => ({ getItemAsync: jest.fn(async () => null), setItemAsync: jest.fn(async () => undefined), deleteItemAsync: jest.fn(async () => undefined) }));
